...
- **Draft-day GM & HC blurbs on the draft replay card.** Each drafted player's
  card now carries a short "War room" write-up from the team's **GM** (value /
  board / process angle, driven by `draftConviction`, `intangiblesWeighting`,
  and `GmQuirk`) and **head coach** (scheme-fit / role / development, naming the
  actual scheme archetype, driven by `qbDevelopment` and `HcQuirk`). The blurbs
  are a window into how that regime evaluates — a `FILM_ROOM_HERMIT` /
  `PROCESS_PURIST` GM reads differently than a `STAR_CHASER` / `COMBINE_OBSESSED`
  one. Generated by the new pure `media/draft-blurbs.ts` (`buildDraftBlurbs`)
  off the Living Voice `voiceSeed` at render time — **not stored** (re-rolls
  identically, varies per playthrough), with **no ingestion of real GM/HC
  wording** (hand-authored phrase banks + seeded `prng.pick`). Length scales by
  round (R1 ~5 sentences → R7 ~1), and a board steal (`overallPick −
  consensusRank` ≥ 28 / ≥ 60) bumps length plus adds a value angle.
- **`DraftPickRecord.prospectProfile` pick-time snapshot.** Drafted prospects
  are pruned from `collegePool` each offseason, so the replay card had no live
  source for a player's position / school / measurables / college production /
  tier. Each pick now snapshots a `DraftProspectProfile` from the chosen
  prospect at pick-build time (no PRNG, draft outcomes unchanged). The replay
  card reads this snapshot (team badge, perceived/real grade pair, combine /
  pro-day, college production) instead of the live pool.

### Changed

- **Inspector: draft replay card expanded** to render the snapshot fields above
  plus the GM + HC "War room" section. Pre-v0.162 drafts have no
  `prospectProfile`, so the card shows a "re-sim a draft" hint and omits the
  blurbs until a fresh draft is advanced.
- **Inspector: team dropdowns alphabetized** (trade partner, GM media-trust,
  scout/audit filters).
