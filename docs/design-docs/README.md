...
| Talent Allocation — starter-caliber players belong to jobs, not to hoards | `docs/design-docs/TALENT_ALLOCATION.md` | **§1–§9 = Candidate D as shipped-uncommitted (D-1a/D-1b `QUALITY_DEPTH_TARGET` + D-3 starting-opportunity + D-2 `releaseSurplusStarters`), on top of v0.189.1. ⚠️ §9's "structural single-pass clearing" diagnosis was INVESTIGATED AND FALSIFIED — read §10, do not implement §9's open question.** **§10 (Opus, 2026-08-04) = the corrected diagnosis + the executable plan, written for Sonnet.** The residual is THREE defects: clustering 15.5%→4.7% is **allocation** (oracle reaches 5.1%, so reachable); zero-starter rooms 22.1%→24.5% is **roster shape** (sim carries 3.13 QBs/roster vs real 2.74 = 18.6% starter-caliber oversupply); ladder 1.18→1.88 is **erosion/distribution** (no permutation reaches it — oracle scores 0.96, WORSE than current). **Root cause of the allocation defect: D-1/D-2/D-3 all gate on `player.tier`, but 48.9% of blocking QBs are tier-invisible vs a 3.2% population rate (15× enrichment) — `talent-score.ts` ranks within-position then destroys it via a 3-yr EWMA + fixed absolute cuts.** Fix = swap the tier gate for `withinPositionPercentiles` (already exported). **DEAD ENDS recorded: multi-round clearing, moving D-2 after the draft (draft supplies 0.4% of blockers), tuning `STARTING_OPPORTUNITY_MAGNITUDE`, Candidate B.** Sequencing (Daniel-approved): Track 2 roster shape FIRST, then Track 1 gate fix; Track 3 → `TALENT_EROSION.md`. Drive **split for size**: `1pwbI8TgqjbjPIEODJXevKpJQBzTprubN` (§1–§9 + supersession banner, 13,176B) + `1yph-pnbh97xeqpyQsRc0-YiS0eOC4O9o` (§10, 14,265B) — both byte-verified; local file is the complete source. Supersedes `1R6BwrnbTSOjoDBUCex1uCdAwvs3A9DSI` and the stray 71B `1xkNzJSub7rw9xiB9G60BZdLtIwFD_wWb` — trash both. |
