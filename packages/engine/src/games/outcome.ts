...
function rollStats(
  ...
  const protEdge = own.passProtection - opp.passRush; // + = clean pocket
  const runEdge = (own.runBlocking + own.rushingCorps) / 2 - opp.runDefense;

  // Passing: a QB/receiving edge over the coverage AND a protected pocket
  // both lift yards; getting out-rushed (negative protEdge) suppresses them.
  const passingMean = 220 + coverageEdge * 1.4 + protEdge * 0.8 + pointsScored * 1.6;
  ...
  // and pressure (opp rush vs own protection) both force giveaways.
  const turnoversMean =
    1.3 + (opp.coverage - own.qbPlay) * 0.016 + (opp.passRush - own.passProtection) * 0.008;
  const turnovers = Math.max(0, Math.round(prng.normal(turnoversMean, 0.85)));

  // Sacks generated by THIS team's defense: own pass rush vs opp protection.
  const sacksMean = 2.4 + (own.passRush - opp.passProtection) * 0.05;
