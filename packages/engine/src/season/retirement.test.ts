...
describe('advanceSeason — retirement integration', () => {
  ...
  it('10-season trajectory: retirements fire, rosters hold 53+16, ages cap at 40, players store bounded, age distribution plausible', () => {
    ...
    for (let season = 1; season <= 10; season++) {
      ...
      // Every team at exactly 53 active + exactly 16 PS after every offseason,
      // through retirement churn. The mandatory-53 floor (v0.186,
      // `enforceRosterFloor`) guarantees the under-53 half — a
      // compliant-but-cap-strapped team (e.g. ATL:48 on this seed at season 8
      // under Season Form D1) gets a restructure-first ladder that frees room
      // for its 53rd body — and the POST_DRAFT_ROSTER cutdown guarantees the
      // over-53 half.
