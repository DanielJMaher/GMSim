...
describe('buildSeasonTimeline', () => {
  ...
  it('opens with College Week 1 (Aug 30), before NFL Week 1 (Sept 7)', () => {
    const first = timeline[0]!;
    expect(first.phase).toBe('COLLEGE_WEEK');
    expect(first.weekIndex).toBe(0);
    expect(formatCalendarDate(first.date)).toBe('2026-08-30');
  });

  it('orders the regular-season opening as CFB W1, CFB W2, NFL W1 (the v0.63 bug)', () => {
    const firstThree = timeline.slice(0, 3).map((s) => ({
      phase: s.phase,
  ...
  it('contains every week + single-shot phase exactly the expected number of times', () => {
    ...
    // Each postseason / offseason phase appears once.
    for (const p of [
    ...
    expect(timeline.length).toBe(17 + 12 + 17);
  });
