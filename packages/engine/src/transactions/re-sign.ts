 ...
 * gate. Pure — exported for tests and the inspector.
 */
export function resignProbability(player: Player, seasonNumber: number): number {
...
export function applyResigningWindow(
  ...
  for (const teamId of teamIds) {
    ...
    for (const player of candidates) {
      const p = resignProbability(player, league.seasonNumber);
